...
```{r}
data <- readRDS("C:/Users/Eric/My Drive/Lab/datasets/EYW/CTR02_R_240516/processing/CTR02_seurat_SCT_CRISPRumi9.rds")
```
...
ifngr1Comp %>%
  filter(padj < 0.1) %>%
  ggplot(aes(y = infected_sig, fill = infected_sig)) +
    geom_bar()
```


```{r}
ifngr1CompDirect <- resLFCIfngr1Tidy %>%
  left_join(infectedSig) %>%
...
ggplot(ifngr1CompDirect, aes(x = log2FoldChange, y = avg_log2FC, color = sig_category)) +
  geom_point()+
  xlim(-4,4) +
  ylim(-4,4) +
  theme(aspect.ratio = 1)
